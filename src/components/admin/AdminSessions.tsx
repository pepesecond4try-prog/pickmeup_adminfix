...
import { Plus, Calendar, Clock, Loader2, Play, Square, RefreshCw, X, AlertCircle } from 'lucide-react';
...
export default function AdminSessions() {
  ...
  const saveSession = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const { data, error: rpcError } = await supabase.rpc('admin_create_session', {
        p_name: name,
      ...
      if (data && !data.success) throw new Error(data.message || 'Failed to create session');
  ...
  const toggleSessionStatus = async (id: string, currentStatus: boolean) => {
    if (!supabase) return;
    try {
      const { error: updateError } = await supabase.from('sessions').update({ is_active: !currentStatus }).eq('id', id);
  ...
  const bulkResetTickets = async (id: string) => {
    ...
    try {
      const { data, error: rpcError } = await supabase.rpc('admin_bulk_reset_tickets', { p_session_id: id });
      if (rpcError) throw rpcError;
      if (data && !data.success) throw new Error(data.message || 'Failed to reset tickets');
      fetchSessions();
    } catch (err: any) {
      console.error('Error resetting tickets:', err);
      alert(`Failed to reset tickets: ${err.message}`);
    }
  };
