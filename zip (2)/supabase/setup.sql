...
BEGIN
    ...
    SELECT is_active INTO user_active FROM users WHERE id = uid;
...
BEGIN
    ...
    -- Auto-generate tickets for active sessions
    IF p_is_active THEN
        INSERT INTO tickets (session_id, user_id)
        SELECT id, new_user_id FROM sessions WHERE is_active = true AND now() < end_time;
    END IF;
...
BEGIN
    ...
    -- Auto-generate tickets for active sessions if user is active
    IF p_is_active THEN
        INSERT INTO tickets (session_id, user_id)
        SELECT id, p_user_id FROM sessions WHERE is_active = true AND now() < end_time
