...
import { Grid, Map, Plus, MessageSquare, User, Settings, Bell, ArrowRight, Check, X, LogOut, Loader2, Copy, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchPings = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load pings');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updatePingStatus = async (id: string, status: 'viewed' | 'picked_up') => {
    if (!supabase) return;
    try {
      const { error: updateError } = await supabase.from('pings').update({ status }).eq('id', id);
      if (updateError) throw updateError;
    } catch (err: any) {
      console.error('Error updating ping:', err);
      alert(`Failed to update ping: ${err.message}`);
    }
  };
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-normal text-white">{ping.users?.display_name || 'Unknown User'}</h3>
      ...
      <div className="w-full max-w-md min-h-screen flex flex-col bg-black relative shadow-none border-x border-neutral-900 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0IiBoZWlnaHQ9IjQiPjxyZWN0IHdpZHRoPSI0IiBoZWlnaHQ9IjQiIGZpbGw9IiMwMDAwMDAiLz48cmVjdCB3aWR0aD0iMSIgaGVpZ2h0PSIxIiBmaWxsPSIjMzMzMzMzIiBmaWxsLW9wYWNpdHk9IjAuMSIvPjwvc3ZnPg==')]">
        ...
        <main className="flex-1 flex flex-col px-6 pt-6 pb-24 gap-8">
          <div className="flex gap-6 text-sm font-medium border-b border-neutral-900 pb-3 overflow-x-auto scrollbar-hide">
            <button onClick={() => setActiveTab('active')} className={`pb-3 -mb-3.5 px-1 whitespace-nowrap transition-colors ${(activeTab === 'active' || activeTab === 'history') ? 'text-white border-b border-white' : 'text-neutral-500 hover:text-neutral-300'}`}>
              Pings
            </button>
              ...
              <>
                {error && (
                  <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 flex items-start gap-3 text-red-400">
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-sm font-medium">Error loading pings</span>
                ...
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-sm font-medium text-neutral-400 uppercase tracking-wider">
                    {activeTab === 'active' ? 'Active Requests' : 'History'}
                  </h2>
                  <button onClick={fetchPings} className="text-xs text-neutral-500 hover:text-white transition-colors flex items-center gap-1">
                ...
                {loading && pings.length === 0 ? (
                  ...
                  <>
                    ...
                    {activeTab === 'active' && activePings.length === 0 && !error && (
                      <div className="text-center text-neutral-500 py-10 text-sm">
                        No active requests.
                      </div>
                    )}
                    {activeTab === 'history' && historyPings.length === 0 && !error && (
                      <div className="text-center text-neutral-500 py-10 text-sm">
                        No history found.
                      </div>
                    )}
