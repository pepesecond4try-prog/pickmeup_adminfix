...
import { Plus, Edit2, Copy, Check, X, Loader2, UserX, UserCheck, RefreshCw, AlertCircle } from 'lucide-react';
  ...
  const saveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        if (data && !data.success) throw new Error(data.message || 'Failed to update user');
      } else {
        ...
        if (rpcError) throw rpcError;
        if (data && !data.success) throw new Error(data.message || 'Failed to create user');
      }
